...
def getFolderContentPng(path):
  onlyPdfFiles = [f for f in listdir(path) if isfile(join(path, f)) and  f.endswith(".png")]
  return onlyPdfFiles

import os
path_to_txt_dir = 'txt/'
if not os.path.exists(path_to_txt_dir):
    os.mkdir (path_to_txt_dir)

def getFolderContentPNG(path):
  return  [f for f in listdir(path) if isfile(join(path, f)) and  f.endswith(".png")]
def createTxt(name,contenu):
  file = open(name, "w") 
  file.write(contenu) 
  file.close()

from os import listdir
imgs=getFolderContentPNG('png/')
for img in imgs:
  txt=image_to_text('png/'+img,lang=fra)
  name=img.replace("png","txt")
  createTxt("txt/"+name,txt)
